...
enum class ConnectivityNetworkCapability(val raw: Int) {
  CELLULAR(0),
  WIFI(1),
  VPN(2),
  UNMETERED(3);
  ...
  companion object {
    fun ofRaw(raw: Int): ConnectivityNetworkCapability? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
private open class ConnectivityPigeonCodec : StandardMessageCodec() {
    ...
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          ConnectivityTransportAvailability.ofRaw(it.toInt())
        }
      }
      130.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          ConnectivityNetworkCapability.ofRaw(it.toInt())
        }
      }
      131.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          ConnectivityTransportSnapshot.fromList(it)
        }
      }
  ...
  override fun writeValue(stream: ByteArrayOutputStream, value: Any?)   {
    when (value) {
      is ConnectivityTransportAvailability -> {
        stream.write(129)
        writeValue(stream, value.raw.toLong())
      }
      is ConnectivityNetworkCapability -> {
        stream.write(130)
        writeValue(stream, value.raw.toLong())
      }
      is ConnectivityTransportSnapshot -> {
        stream.write(131)
...
interface ConnectivityApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: ConnectivityApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.immich_mobile.ConnectivityApi.start$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              api.start()
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.immich_mobile.ConnectivityApi.stop$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              api.stop()
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.immich_mobile.ConnectivityApi.dispose$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              api.dispose()
...
class ConnectivityFlutterApi(private val binaryMessenger: BinaryMessenger, private val messageChannelSuffix: String = "") {
  companion object {
    /** The codec used by ConnectivityFlutterApi. */
    val codec: MessageCodec<Any?> by lazy {
      ConnectivityPigeonCodec()
    }
  }
  fun onTransportChanged(snapshotArg: ConnectivityTransportSnapshot, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.immich_mobile.ConnectivityFlutterApi.onTransportChanged$separatedMessageChannelSuffix"
    ...
    channel.send(listOf(snapshotArg)) {
      if (it is List<*>) {
        ...
        callback(Result.failure(ConnectivityPigeonUtils.createConnectionError(channelName)))
      } 
    }
  }
}
