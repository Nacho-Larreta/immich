...
class MergedAssetDrift extends i1.ModularAccessor {
  ...
  i0.Selectable<MergedAssetResult> mergedAsset({
    ...
    return customSelect(
      'SELECT rae.id AS remote_id, (SELECT lae.id FROM local_asset_entity AS lae WHERE lae.checksum = rae.checksum LIMIT 1) AS local_id, rae.name, rae.type, rae.created_at AS created_at, rae.updated_at, rae.width, rae.height, rae.duration_ms, rae.is_favorite, rae.thumb_hash, rae.checksum, rae.owner_id, rae.live_photo_video_id, 0 AS orientation, rae.stack_id, NULL AS i_cloud_id, NULL AS latitude, NULL AS longitude, NULL AS adjustmentTime, rae.is_edited, 0 AS playback_style FROM remote_asset_entity AS rae LEFT JOIN stack_entity AS se ON rae.stack_id = se.id WHERE ?1 != 0 AND rae.deleted_at IS NULL AND rae.visibility = 0 AND rae.owner_id IN ($expandeduserIds) AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT (SELECT remote_match.id FROM remote_asset_entity AS remote_match WHERE remote_match.checksum = lae.checksum AND remote_match.owner_id IN ($expandeduserIds) AND remote_match.deleted_at IS NULL AND remote_match.visibility = 0 AND(remote_match.stack_id IS NULL OR remote_match.id = (SELECT primary_asset_id FROM stack_entity WHERE id = remote_match.stack_id))ORDER BY remote_match.id LIMIT 1) AS remote_id, lae.id AS local_id, lae.name, lae.type, lae.created_at AS created_at, lae.updated_at, lae.width, lae.height, lae.duration_ms, lae.is_favorite, NULL AS thumb_hash, lae.checksum, NULL AS owner_id, NULL AS live_photo_video_id, lae.orientation, NULL AS stack_id, lae.i_cloud_id, lae.latitude, lae.longitude, lae.adjustment_time, 0 AS is_edited, lae.playback_style FROM local_asset_entity AS lae WHERE ?1 != 2 AND(?1 = 0 OR NOT EXISTS (SELECT 1 FROM remote_asset_entity AS remote_match WHERE remote_match.checksum = lae.checksum AND remote_match.owner_id IN ($expandeduserIds) AND remote_match.deleted_at IS NULL AND remote_match.visibility = 0 AND(remote_match.stack_id IS NULL OR remote_match.id = (SELECT primary_asset_id FROM stack_entity WHERE id = remote_match.stack_id))))ORDER BY created_at DESC ${generatedlimit.sql}',
      variables: [
        i0.Variable<int>(sourceFilter),
