...
) async -> Bool
{
    ...
    if !isResume {
        return await withCheckedContinuation { continuation in
            ...
            dataTask = UrlSessionDelegate.foregroundSession().dataTask(with: baseRequest) { (data, response, error) in
                guard BackgroundDownloaderRequestContextBridge.shouldProcessCallback(for: dataTask) else {
                    dataTask.cancel()
                    continuation.resume(returning: false)
                    return
                }
                BackgroundDownloaderRequestContextBridge.complete(dataTask)
