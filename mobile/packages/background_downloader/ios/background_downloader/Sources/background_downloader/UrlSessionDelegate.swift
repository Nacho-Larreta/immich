...
public class UrlSessionDelegate : NSObject, URLSessionDelegate, URLSessionDownloadDelegate, URLSessionDataDelegate {
    ...
    static var foregroundUrlSession: URLSession?
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, willBeginDelayedRequest request: URLRequest) async -> (URLSession.DelayedRequestDisposition, URLRequest?) {
        guard let securedRequest = BackgroundDownloaderRequestContextBridge.securedDelayedRequest(
            request,
            for: task
        ) else {
            return (.cancel, nil)
        ...
        guard let bgdTask = getTaskFrom(urlSessionTask: task)
            ...
            return (.useNewRequest, securedRequest)
        }
        ...
        if securedRequest.allHTTPHeaderFields != nil {
            for (key, value) in securedRequest.allHTTPHeaderFields! {
                newRequest.setValue(value, forHTTPHeaderField: key)
            }}
        for (key, value) in newTask.headers {
            newRequest.setValue(value, forHTTPHeaderField: key)
        }
        newRequest.httpMethod = securedRequest.httpMethod
        newRequest.httpBody = securedRequest.httpBody
        newRequest.allowsCellularAccess = securedRequest.allowsCellularAccess
        guard let finalRequest = BackgroundDownloaderRequestContextBridge.securedDelayedRequest(
            newRequest,
            for: task
        ) else {
            return (.cancel, nil)
        }
        return (URLSession.DelayedRequestDisposition.useNewRequest, finalRequest)
    }
    ...
    public func urlSession(
        ...
        completionHandler(
            BackgroundDownloaderRequestContextBridge.securedRedirect(request, for: task)
        )
    }
    ...
    public func urlSession(
        ...
        BackgroundDownloaderRequestContextBridge.handleChallenge(
            session: session,
            challenge: challenge,
            task: nil,
    ...
    public func urlSession(
        ...
        guard BackgroundDownloaderRequestContextBridge.shouldProcessCallback(for: task) else {
            task.cancel()
            return
        }
        BackgroundDownloaderRequestContextBridge.complete(task)
    ...
    public func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        guard BackgroundDownloaderRequestContextBridge.shouldProcessCallback(for: downloadTask) else {
            downloadTask.cancel()
            return
        }
    ...
    {
        guard BackgroundDownloaderRequestContextBridge.shouldProcessCallback(for: dataTask) else {
            dataTask.cancel()
            return
        }
    ...
    static func createUrlSession() -> Void {
        ...
        BackgroundDownloaderRequestContextBridge.secureConfiguration(config)
        ...
        session.getAllTasks { tasks in
            _ = BackgroundDownloaderRequestContextBridge.cancelRestoredTasks(tasks)
        }
    }
    ...
    static func session(for request: URLRequest) -> URLSession {
        guard
            let url = request.url,
            BackgroundDownloaderRequestContextBridge.executionMode(for: url) == .foreground
        else {
            createUrlSession()
            return urlSession!
        }
    ...
    static func foregroundSession() -> URLSession {
        if let foregroundUrlSession {
            return foregroundUrlSession
        }
        let config = URLSessionConfiguration.default
        BackgroundDownloaderRequestContextBridge.secureConfiguration(config)
        let session = URLSession(configuration: config, delegate: instance, delegateQueue: nil)
        foregroundUrlSession = session
        return session
    }
    
    /// Return all tasks in this urlSession
    static func getAllTasks() async -> [Task] {
        return await getAllUrlSessionTasks().compactMap({ getTaskFrom(urlSessionTask: $0) })
    }
    ...
    static func getAllUrlSessionTasks(group: String? = nil) async -> [URLSessionTask] {
        ...
        let backgroundTasks = await UrlSessionDelegate.urlSession?.allTasks ?? []
        let foregroundTasks = await UrlSessionDelegate.foregroundUrlSession?.allTasks ?? []
        let urlSessionTasks = backgroundTasks + foregroundTasks
        if (group == nil) { return urlSessionTasks }
