...
public class BDPlugin: NSObject, FlutterPlugin, UNUserNotificationCenterDelegate {
    ...
    public func doEnqueue(taskJsonString: String, notificationConfigJsonString: String?, resumeDataAsBase64String: String) async -> Bool {
        ...
        let securedRequest = preparedRequest?.request ?? baseRequest
        let requestContext = preparedRequest?.context
        if isParallelDownloadTask(task: task) {
            return await scheduleParallelDownload(
                task: task,
                taskDescription: taskDescription,
                baseRequest: securedRequest,
                resumeData: resumeDataAsBase64String,
                requestContext: requestContext
            )
        } else if isDownloadTask(task: task) || isDataTask(task: task)
        {
            return await scheduleDownload(
                task: task,
                taskDescription: taskDescription,
                baseRequest: securedRequest,
    ...
    private func scheduleDownload(
        ...
        requestContext: BackgroundDownloaderRequestContextSnapshot?
    ) async -> Bool {
        if resumeData != nil && BackgroundDownloaderRequestContextBridge.isInstalled {
            os_log("Rejecting credentialed resume data for taskId %@", log: log, type: .info, task.taskId)
            return false
        }
        ...
        let urlSession = UrlSessionDelegate.session(for: request)
        let urlSessionDownloadTask = resumeData == nil ? urlSession.downloadTask(with: request) : urlSession.downloadTask(withResumeData: resumeData!)
        urlSessionDownloadTask.taskDescription = taskDescription
        urlSessionDownloadTask.priority = 1 - Float(task.priority) / 10
        guard BackgroundDownloaderRequestContextBridge.resume(
    ...
    private func scheduleUpload(
        ...
        if isBinaryUploadTask(task: task) {
            ...
            let urlSessionUploadTask = urlSession.uploadTask(with: request, fromFile: uploadFileUrl)
            ...
            let urlSessionUploadTask = urlSession.uploadTask(with: request, fromFile: uploader.outputFileUrl())
            urlSessionUploadTask.taskDescription = taskDescription
            urlSessionUploadTask.priority = 1 - Float(task.priority) / 10
    ...
    private func methodAllTasks(call: FlutterMethodCall, result: @escaping FlutterResult) async {
        ...
        let urlSessionTasks = await UrlSessionDelegate.getAllUrlSessionTasks()
