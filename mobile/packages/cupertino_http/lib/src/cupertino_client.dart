...
import 'streaming_task_lifecycle.dart';
...
class CupertinoClient extends BaseClient {
  ...
  late final OwnedStreamingTasks<StreamingTask> _streamingTasks =
      OwnedStreamingTasks<StreamingTask>(
        cancelAndWait: (task) => task.cancelAndWait(),
        completed: (task) => task.completed,
      );
  late final OwnedStreamingTasks<_BufferedTask> _bufferedTasks =
      OwnedStreamingTasks<_BufferedTask>(
        cancelAndWait: (task) => task.cancelAndWait(),
        completed: (task) => task.completed,
      );
  ...
  void close() {
    unawaited(
      Future.wait([
        _streamingTasks.closeAndDrain(),
        _bufferedTasks.closeAndDrain(),
      ]),
    );
    if (_ownsSession) {
  ...
  Future<StreamedResponse> _sendBuffer(
    ...
    late final _BufferedTask bufferedTask;

    final nativeTask = session.dataTaskWithCompletionHandler(urlRequest, (
      data,
      urlResponse,
      error,
    ) {
      try {
        ...
        if (!completer.isCompleted) {
          completer.completeError(callbackError, callbackStack);
        }
      } finally {
        if (nsStream?.streamStatus != NSStreamStatus.NSStreamStatusClosed) {
          nsStream?.close();
    ...
    bufferedTask = _BufferedTask(nativeTask, lifecycle);
    if (!_bufferedTasks.admit(bufferedTask)) {
      if (nsStream?.streamStatus != NSStreamStatus.NSStreamStatusClosed) {
        nsStream?.close();
      }
      await cancelRejectedTaskAndWait(
        cancelAndWait: bufferedTask.cancelAndWait,
        callbackSettlement: completer.future.then<void>(
          (_) {},
          onError: (_) {},
        ),
      );
    ...
    if (request case Abortable(:final abortTrigger?)) {
      unawaited(abortTrigger.whenComplete(bufferedTask.cancel));
    }
...
final class _BufferedTask {
  const _BufferedTask(this._nativeTask, this._lifecycle);

  final URLSessionTask _nativeTask;
  final StreamingTaskLifecycle _lifecycle;

  Future<void> get completed => _lifecycle.completed;

  void resume() => _nativeTask.resume();

  void cancel() => unawaited(cancelAndWait());

  Future<void> cancelAndWait() => _lifecycle.cancelAndWait(_nativeTask.cancel);
}
